...
def input():
    ...
    return render_template("a6-4in.html", title="イベント検索", last_event_id=last_event_id)
...
def output():
    ...
    try:
        ...
        event = {"name": name, "location": location, "date": date}
    except ValueError:
        message = "整数値を入力してください"
        return render_template("a6-4out.html", message=message, event=None)
    except IndexError:
        message = "有効なidを入力してください"
        return render_template("a6-4out.html", message=message, event=None)
    
    response = make_response(render_template("a6-4out.html", event=event, message=""))
